    ...
    <div class="grid grid-cols-2 gap-5 p-5">
        ...
        <div class="grid grid-cols-3 gap-2 h-50 border border-gray-500 p-4 rounded-lg">
            ...
            <div class="col-span-1 text-center flex flex-col justify-around items-center">
                ...
                @if(! \App\Models\review::where('request_id', $request->id)->exists() && $request->status == 'finished')
                <a href="{{$request->id}}/reviews/create" class="cursor-pointer w-fit px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Review</a>
                @endif
            </div>
        </div>
        @endforeach
